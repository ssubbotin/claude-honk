...
- `.claude-plugin/plugin.json` — plugin manifest. Points to `./hooks/hooks.json` via the `hooks` field.
...
- `hooks/hooks.json` — registers the same command (`${CLAUDE_PLUGIN_ROOT}/scripts/play-honk.sh`) under `Notification`, `PermissionRequest`, and `Stop`. Each event gets its own array entry; do **not** consolidate them into one entry under multiple events — Claude Code rejected that as a duplicate-hooks error (see commit `eeeb171`).
